        ...
        if self.cache.qsize() > CACHE_LEN / 2:  # 出队更新下载
            ...
            if self.waitDownload.qsize() > WAIT_DOWNLOAD:
                while self.waitDownload.qsize() > 0:
                    ...
                    t = threading.Thread(target=downloadTorrent.download_metadata,
        ...
        try:
            ...
            ret = self.dbcurr.execute(
                ...
                (info['info_hash'], info['category'], info['data_hash'], info['name'], info['extension'],
                 info['classified'],
                 info['source_ip'], info['tagged'], info['length'], info['create_time'], info['last_seen'],
                 info['requests']))
            if self.count P == 0:
                self.dbconn.commit()
            ...
            print
            self.name, 'save error', self.name, info
            traceback.print_exc()
            return
