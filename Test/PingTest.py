...
class PingTools:
    ...
    def getEnableIPS(StartIP, EndIP):
        ...
        if int(IP1)> 255 or int(IP1) < 0 or int(IP2) >255 or int(IP2) < 0 or int(IP3) >255 or \
            int(IP3) < 0  or int(IP3) >255 or int(IP3) < 0 or int(EndIP.split('.')[0]) <0 or int(EndIP.split('.')[0])>255 or \
                int(EndIP.split('.')[1]) < 0 or int(EndIP.split('.')[1]) > 255 or int(EndIP.split('.')[2]) <0 or int(EndIP.split('.')[2])>255 \
